...
function getSharedPhotos(user_id) {
  $.ajax({
    ...
    dataType: "JSON",
    success: (data) => {
      if (data.shared.length > 59){
        $(".shared-photos-list").html(data.shared);
      } else {
        $(".shared-photos-list").html("<p class=\"message-hint\"><span>Nada compartilhado ainda</span></p>");
      }
    },
    error: () => {
      console.error("Server error, check your response");
...
console.log("aaaaa :: "+messagesContainer.css("opacity", "1"));